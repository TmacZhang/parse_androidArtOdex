...
public class ParseOdex {
    ...
    public static void main(String[] args) {
        ...
        byte[] fileByteArys = Utils.readFile("so/oat.dex");
        ...
        /**
         * 先解析so文件 然后初始化AddSection中的一些信息 最后在AddSection
         */
        parseSo(fileByteArys);

        // 初始化AddSection中的变量
        AddSection.sectionHeaderOffset = Utils.byte2Int(type_32.hdr.e_shoff);
        AddSection.stringSectionInSectionTableIndex = Utils
                .byte2Short(type_32.hdr.e_shstrndx);
        ...
        int lastLoadVaddr = Utils.byte2Int(type_32.phdrList
                .get(AddSection.lastLoadInPHIndex).p_vaddr);
        int lastLoadMem = Utils.byte2Int(type_32.phdrList
                .get(AddSection.lastLoadInPHIndex).p_memsz);
        int lastLoadAlign = Utils.byte2Int(type_32.phdrList
                .get(AddSection.lastLoadInPHIndex).p_align);
        AddSection.addSectionStartAddr = Utils.align(lastLoadVaddr
                + lastLoadMem, lastLoadAlign);
        System.out.println("start addr hex:"
                + Utils.bytes2HexString(Utils
                        .int2Byte(AddSection.addSectionStartAddr)));

        /**
         * 添加一个Section 以下的顺序不可乱，不然会出错的 1、添加一个Section Header
         * 2、直接在文件的末尾追加一个section 3、修改String段的长度 4、修改Elf Header中的section count
         */
    ...
    private static void parseSo(byte[] fileByteArys) {
         ...
         * ，这个段的大小和偏移地址，而SymbolTable的结构大小是固定的16个字节 //那么这里的数目=大小/结构大小
         * //首先在SectionHeader中查找到dynsym段的信息 int offset_sym = 0; int total_sym =
         * 0; for(elf32_shdr shdr : type_32.shdrList){
         * if(Utils.byte2Int(shdr.sh_type) == ElfType32.SHT_DYNSYM){ total_sym =
         * Utils.byte2Int(shdr.sh_size); offset_sym =
         * Utils.byte2Int(shdr.sh_offset); break; } } int num_sym = total_sym /
         * 16; System.out.println("sym num="+num_sym);
         * parseSymbolTableList(fileByteArys, num_sym, offset_sym);
         * type_32.printSymList();
         * 
         * //读取字符串表信息(String Table) System.out.println();
         * System.out.println("+++++++++++++++++++Symbol Table++++++++++++++++++"
         * );
         ...
         * //首先获取每个段的name的字符串大小 int prename_len = 0; int[] lens = new
         * int[type_32.shdrList.size()]; int total = 0; for(int
         * i=0;i<type_32.shdrList.size();i++){
         * if(Utils.byte2Int(type_32.shdrList.get(i).sh_type) ==
         * ElfType32.SHT_STRTAB){ int curname_offset =
         * Utils.byte2Int(type_32.shdrList.get(i).sh_name); lens[i] =
         * curname_offset - prename_len - 1; if(lens[i] < 0){ lens[i] = 0; }
         * total += lens[i]; System.out.println("total:"+total); prename_len =
         * curname_offset; //这里需要注意的是，最后一个字符串的长度，需要用总长度减去前面的长度总和来获取到 if(i ==
         * (lens.length - 1)){
         * System.out.println("size:"+Utils.byte2Int(type_32.
         * shdrList.get(i).sh_size)); lens[i] =
         * Utils.byte2Int(type_32.shdrList.get(i).sh_size) - total - 1; } } }
         * for(int i=0;i<lens.length;i++){ System.out.println("len:"+lens[i]); }
         * //上面的那个方法不好，我们发现StringTable中的每个字符串结束都会有一个00(传说中的字符串结束符)，
         * 那么我们只要知道StringTable的开始位置，然后就可以读取到每个字符串的值了
         */
    }
    ...
    private static elf32_phdr parseProgramHeader(byte[] header) {
        /**
         * public int p_type; public int p_offset; public int p_vaddr; public
         * int p_paddr; public int p_filesz; public int p_memsz; public int
         * p_flags; public int p_align;
         */
     ...
     * 解析Symbol Table内容
     */
    public static void parseSymbolTableList(byte[] header, int header_count,
            int offset) {
    ...
    private static ElfType32.Elf32_Sym parseSymbolTable(byte[] header) {
        /**
         * public byte[] st_name = new byte[4]; public byte[] st_value = new
         * byte[4]; public byte[] st_size = new byte[4]; public byte st_info;
         * public byte st_other; public byte[] st_shndx = new byte[2];
         */
