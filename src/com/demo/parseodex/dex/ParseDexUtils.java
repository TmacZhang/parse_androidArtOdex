...
public class ParseDexUtils {
    ...
    public static void parseTypeIds(byte[] srcByte) {
        ...
        for (TypeIdsItem item : typeIdsList) {
            System.out
                    .println("typeStr:" + stringList.get(item.descriptor_idx));
        }
    }
    ...
    public static void parseProtoIds(byte[] srcByte) {
        ...
        for (ProtoIdsItem item : protoIdsList) {
            System.out.println("proto:" + stringList.get(item.shorty_idx) + ","
    ...
    public static void parseFieldIds(byte[] srcByte) {
        ...
        for (FieldIdsItem item : fieldIdsList) {
            ...
            System.out.println("class:" + stringList.get(classIndex) + ",name:"
                    + stringList.get(item.name_idx) + ",type:"
    ...
    public static void parseMethodIds(byte[] srcByte) {
        ...
        for (MethodIdsItem item : methodIdsList) {
            ...
            System.out.println("class:" + stringList.get(classIndex) + ",name:"
                    + stringList.get(item.name_idx) + ",proto:" + parameters);
        }
    ...
    public static void parseClassIds(byte[] srcByte) {
        System.out.println("classIdsOffset:"
                + Utils.bytesToHexString(Utils.int2Byte(classIdsOffset)));
    ...
    private static ClassDataItem parseClassDataItem(byte[] srcByte, int offset) {
        ...
        for (int i = 0; i < item.static_fields_size; i++) {
            /**
             * public int filed_idx_diff; public int access_flags;
             */
    ...
    private static String getString(byte[] srcByte, int startOff) {
         ...
         * ))); List<Byte> byteList = new ArrayList<Byte>(); //第一个字节是长度，所以过滤一下
         * byte b = srcByte[startOff+1]; int index = 0; while(b != 0){
         * byteList.add(b); index++; b = srcByte[startOff+index]; } byte[]
         * byteAry = new byte[byteList.size()+1]; for(int
         * i=0;i<byteList.size();i++){ byteAry[i] = byteList.get(i); }
         * byteAry[byteList.size()] = 0; String result = ""; try{ result = new
