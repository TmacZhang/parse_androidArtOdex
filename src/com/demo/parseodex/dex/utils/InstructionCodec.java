...
public final class InstructionCodec {
    ...
    public static int getTargetByte(int target, int baseAddress) {
        ...
        if (relativeTarget != (byte) relativeTarget) {
            try {
                throw new Exception("Target out of range: " + (relativeTarget)
    ...
    public static int getLiteralByte(long literal) {
        if (literal != (byte) literal) {
            try {
                throw new Exception("Literal out of range: " + (literal));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static short getAUnit(int a) {
        if ((a & ~0xffff) != 0) {
            try {
                throw new Exception("Register A out of range: " + (a));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static short getBUnit(int b) {
        if ((b & ~0xffff) != 0) {
            try {
                throw new Exception("Register B out of range: " + (b));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * 返回一个指令的长度，返回值是几个short，一个short是4位,一个指令是2位
     * 
     * @param instructionFormat
     * @return
     */
    public static int getInstructionFormatLength(int instructionFormat) {
        int length = 0;
